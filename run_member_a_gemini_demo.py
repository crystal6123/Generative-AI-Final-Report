...
def main() -> None:
    ...
    if not api_key:
        print("找不到 GEMINI_API_KEY。請在專案根目錄建立 .env 或在 PowerShell 設定：")
        print('$env:GEMINI_API_KEY="你的 Gemini API key"')
        print('可選：$env:GEMINI_MODEL="gemini-2.5-flash"')
        return

    if api_key.startswith("請把這裡換成") or api_key == "your_api_key_here":
        print("請先把 .env 裡的 GEMINI_API_KEY 換成真正的 API key。")
    ...
    print(f"專案根目錄：{ROOT}")
    print(f"資料庫：{db_path}")
    print(f"Gemini model：{os.environ.get('GEMINI_MODEL', 'gemini-3.5-flash')}")
